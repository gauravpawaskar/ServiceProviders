...
function set_up_service(service_name, client_id, client_secret){
    ...
    setup_xhr.onreadystatechange = function() { 
        if (setup_xhr.readyState === XMLHttpRequest.DONE && setup_xhr.status === 200) {
    ...
    setup_xhr.send(JSON.stringify({ "service": service_name, "client_id": client_id, "client_secret": client_secret }));
}
